...
import { Heart, Building2, TestTube, Activity, Menu, ArrowRight, PlayCircle, Star, Video, ClipboardList, Baby, Brain, Bone, Droplet, Eye } from 'lucide-react';

export default function LandingPage() {
  const navigate = useNavigate();
  const slides = [
    { id: 'hero-slide-0', title: 'Compassionate Care', subtitle: 'Patient-Centered', icon: <Heart className="w-6 h-6" /> },
    { id: 'hero-slide-1', title: 'Premium Facility', subtitle: 'Modern Environment', icon: <Building2 className="w-6 h-6" /> },
    { id: 'hero-slide-2', title: 'Expert Team', subtitle: 'Advanced Diagnostics', icon: <TestTube className="w-6 h-6" /> }
  ];
  ...
  return (
    <div className="text-on-surface antialiased selection:bg-secondary/20 selection:text-primary-container min-h-screen flex flex-col w-full relative">
      ...
      <motion.header 
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className="bg-surface/60 backdrop-blur-xl border-b border-outline-variant/30 shadow-sm w-full top-0 z-50 transition-all duration-300 sticky"
      >
      ...
      <main className="flex-grow flex flex-col relative overflow-hidden z-10">
        ...
        <section className="relative pt-12 md:pt-24 pb-20 px-margin-mobile md:px-margin-desktop max-w-container-max-width mx-auto w-full z-10 flex-grow flex flex-col justify-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-gutter items-center">
            ...
            <motion.div 
              ...
              className="md:col-span-6 flex flex-col gap-stack-lg z-10"
            >
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 mt-4">
                <motion.button 
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="bg-primary-container text-white px-8 py-4 rounded-[12px] font-label-md text-label-md shadow-premium-glass flex items-center gap-2 group"
                >
                  Book Now
                ...
                <Link className="font-label-md text-label-md text-primary-container hover:text-secondary flex items-center gap-2 group transition-colors duration-200 py-4" to="/departments">
                  Learn more
                  <PlayCircle className="w-6 h-6 text-outline group-hover:text-secondary transition-colors duration-200" />
                </Link>
              </div>
            ...
            <motion.div 
              ...
              className="md:col-span-6 relative mt-12 md:mt-0 flex justify-center md:justify-end h-[500px] md:h-auto md:min-h-[500px]"
            >
              ...
              <div className="relative w-full max-w-[400px] aspect-[4/5] z-10 group mt-8 md:mt-0 mr-4 md:mr-12">
                {[
                  ...
                  return (
                    <div 
                      key={index}
                      className={`absolute inset-0 w-full h-full rounded-[32px] overflow-hidden border-4 border-white/50 bg-white/20 backdrop-blur-md transition-all duration-700 ease-out`}
                      style={{
                        zIndex: 30 - offset * 10,
              ...
              <div className={`absolute bottom-12 -left-8 md:left-4 bg-white/70 backdrop-blur-xl rounded-[20px] p-4 flex items-center gap-4 z-40 animate-[bounce_4s_infinite_alternate] shadow-premium-glass border border-white/40 transition-opacity duration-500 ${fade ? 'opacity-100' : 'opacity-0'}`}>
        ...
        {/* Services Section */}
        <section className="py-section-gap px-margin-mobile md:px-margin-desktop max-w-container-max-width mx-auto w-full z-10">
          <motion.div 
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 auto-rows-[250px]">
            <motion.div 
              ...
              className="md:col-span-2 bg-white/70 backdrop-blur-xl rounded-[32px] p-8 shadow-premium-glass relative overflow-hidden group flex flex-col justify-end border border-white hover:border-secondary-fixed/50 transition-colors duration-300"
            >
            ...
            <div className="md:col-span-1 flex flex-col gap-6">
              <motion.div 
                ...
                className="flex-1 bg-surface-container-low/70 backdrop-blur-xl rounded-[24px] p-6 shadow-premium-glass relative overflow-hidden group hover:bg-surface-container transition-colors duration-300 flex flex-col justify-between"
              >
              ...
              <motion.div 
                whileHover={{ y: -5, scale: 1.02 }}
                className="flex-1 bg-primary-container/90 backdrop-blur-xl rounded-[24px] p-6 shadow-premium-glass relative overflow-hidden group flex flex-col justify-between text-white border border-primary-container"
              >
        ...
        <section className="py-section-gap px-margin-mobile md:px-margin-desktop max-w-container-max-width mx-auto w-full z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-gutter">
            {[
              { title: 'Cardiology', desc: 'Expert care for your heart, from routine checkups to advanced surgical procedures.', icon: <Activity className="w-6 h-6" /> },
              ...
              <motion.div 
                ...
                whileHover={{ y: -8 }}
                className="bg-white/70 backdrop-blur-xl rounded-[24px] p-8 shadow-premium-glass border border-white hover:shadow-premium-hover transition-all duration-300"
              >
        ...
        <section className="py-section-gap px-margin-mobile md:px-margin-desktop max-w-container-max-width mx-auto w-full mb-section-gap z-10">
