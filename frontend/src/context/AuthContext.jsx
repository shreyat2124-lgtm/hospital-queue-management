...
// make context
// a global variable for React. Any component wrapped inside <AuthProvider> can access the user, token, and login/logout functions without passing props down manually.
const AuthContext = createContext(null)

//  Provider Component 
// wraps the entire app (in App.jsx). It manages user, token, login and logout
...
export function AuthProvider({ children }) {
  ...
  // If yes, we call GET /api/auth/me to verify it's still valid and get the user's info. If the token is expired/invalid, the API will return 401 and our interceptor will clear it.
  
  // This is why we can stay logged in after refreshing the page.
  useEffect(() => {
  ...
  const login = async (email, password) => {
    ...
    // Register a new user account, then auto-login
  ...
  // After this, the user is fully logged out and PrivateRoute will redirect them to /login.
  const logout = () => {
    localStorage.removeItem('token')
    localStorage.removeItem('user')
    setToken(null)
...
//  Custom Hook 
// Instead of writing useContext(AuthContext) everywhere, we create a shortcut: useAuth().
